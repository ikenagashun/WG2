...
Java_com_example_facedetect_MainActivity_featureAnalyzer(
    ...
    float iii;
    iii = (float) screenSize / (float) img.rows;
    //iii = 1;
        ...
        for( int i=0; i<dtfaces.rows; i++)
        {
            ...
            for (size_t j = 0; j < PFeature.size(); j++) {
                ...
                if (score >= 0.3 ){
                    ...
                    // 矩形表示
                    rectangle(*matDst,
                              Rect2i(int(dtfaces.at<float>(i, 0))*(OUT_MAGNIFICATION*iii),
                                     int(dtfaces.at<float>(i, 1))*(OUT_MAGNIFICATION*iii),
                                     int(dtfaces.at<float>(i, 2))*(OUT_MAGNIFICATION*iii),
                                     int(dtfaces.at<float>(i, 3))*(OUT_MAGNIFICATION*iii)),
                              Scalar(255, 255, 255), 5);
                    putText(*matDst, PFeature[j].PName,
                            Point(int(dtfaces.at<float>(i, 0))*(OUT_MAGNIFICATION*iii),int(dtfaces.at<float>(i, 1))*(OUT_MAGNIFICATION*iii)),
                            FONT_HERSHEY_SIMPLEX,
                            3,
                            Scalar(255,255,255),
                            5);

                    no = PFeature[j].TagName;
